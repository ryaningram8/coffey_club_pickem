...
class NotificationPrefsCubit extends Cubit<NotificationPrefsState> {
  NotificationPrefsCubit({required NotificationRepository notificationRepository})
  ...
  Future<void> setPickRemindersEnabled(bool value) => _patchPickReminders({'enabled': value});
  Future<void> setPickRemindersPush(bool value) => _patchPickReminders({'push': value});
  Future<void> setPickRemindersEmail(bool value) => _patchPickReminders({'email': value});
  Future<void> setHoursBeforeDeadline(int hours) =>
      _patchPickReminders({'hoursBeforeDeadline': hours});

  Future<void> setResultsEnabled(bool value) => _patchResults({'enabled': value});
  ...
  Future<void> _patchResults(Map<String, dynamic> patch) async {
    ...
    if (current is! NotificationPrefsLoaded) return;
    try {
      final updated = await _repository.updatePrefs(resultsNotifications: patch);
      emit(NotificationPrefsState.loaded(prefs: updated));
    } catch (e) {
      emit(current.copyWith(errorMessage: e.toString()));
    }
