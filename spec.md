...
- [~] Invite management screen (create, copy code/link, see who used it) — `InviteManagementScreen`, reachable via a new mail icon in the Commissioner AppBar; lets a commissioner pick a pool from a dropdown (new `GET /seasons` list endpoint, since only "the active one" existed before), generate a single targeted-by-email or an anonymous batch (up to 25) of codes, and see each code's status (unused / reserved for an email / redeemed by whom / expired), with copy-code and (web-only) copy-signup-link actions. `flutter analyze` is clean across `coffey_ui`/`web`/`mobile`; not interactively exercised in-browser this pass — this session's browser tool couldn't render screenshots or drive coordinate-based clicks against the Flutter web build (confirmed environment-wide: screenshot times out, synthetic accessibility-enable clicks are ignored, coordinate clicks refuse without a screenshot first), the same class of gap noted for the Phase 4 notification-prefs screen above. Pending a manual check.
- [~] `POST /invitations/:code/redeem` — not in the original spec; lets an *existing* logged-in user join an additional pool/season without creating a new account. Needed because a `SeasonMembership` join table (also not in the original spec) was added to make multiple concurrent pools possible — previously every `User` implicitly saw one single global "active season," with no notion of which pool they'd actually joined. `getActiveSeason`/`getCurrentWeek` now resolve per-user against `SeasonMembership` instead of globally, and `submitPicks` now 403s a pick submitted for a pool the user hasn't joined. Existing users were backfilled into whatever season(s) already existed so this didn't break anyone. No pool-switcher UI yet — if a user ends up in two simultaneously active-or-upcoming pools, resolution is deterministic (active beats upcoming, newest wins ties) but not user-choosable.
